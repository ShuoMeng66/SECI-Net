...
def apply_sequence_mask(sequence: Tensor, attention_mask: Optional[Tensor]) -> Tensor:
...
class FactorizedSpatialTemporalEncoding(nn.Module):
    ...
    def forward(self, x: Tensor, time_values: Optional[Tensor] = None) -> Tensor:
...
class DifferentialMultiHeadAttention(nn.Module):
    ...
    def _masked_scores(self, queries: Tensor, keys: Tensor, key_padding_mask: Optional[Tensor]) -> Tensor:
        scores = torch.matmul(queries, keys.transpose(-2, -1)) / math.sqrt(self.head_dim)
...
class BlockSparseMultiHeadAttention(nn.Module):
    ...
    def _compute_salient_blocks(self, x: Tensor, attention_mask: Tensor) -> Tensor:
        ...
        if pad_len > 0:
            x = torch.cat(
                [x, x.new_zeros(batch_size, pad_len, embed_dim)],
                dim=1,
            )
...
class StackedCustomLSTM(nn.Module):
    def __init__(
        ...
        for _ in range(num_layers):
            self.layers.append(CustomLSTMCell(current_input_dim, hidden_dim))
    ...
    def forward(self, x: Tensor, lengths: Tensor) -> tuple[Tensor, Tensor]:
        ...
        for time_step in range(seq_len):
            ...
            active_mask = (time_step < lengths).float().unsqueeze(1)

            for layer_index, layer in enumerate(self.layers):
                h_prev = hidden_states[layer_index]
                c_prev = cell_states[layer_index]
                h_t, c_t = layer(layer_input, h_prev, c_prev)

                h_t = active_mask * h_t + (1.0 - active_mask) * h_prev
                c_t = active_mask * c_t + (1.0 - active_mask) * c_prev

                hidden_states[layer_index] = h_t
                cell_states[layer_index] = c_t
...
class HybridTextClassifier(nn.Module):
    def __init__(
        ...
        self.sequence_encoder = StackedCustomLSTM(
            input_dim=embed_dim,
        ...
        self.classifier = nn.Sequential(
            nn.Linear(embed_dim + lstm_hidden_dim, ffn_hidden_dim),
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(ffn_hidden_dim, num_classes),
        )

    def masked_mean_pooling(self, sequence: Tensor, attention_mask: Tensor) -> Tensor:
        mask = attention_mask.unsqueeze(-1).float()
        masked_sequence = sequence * mask
        pooled = masked_sequence.sum(dim=1) / mask.sum(dim=1).clamp_min(1.0)
        return pooled
    ...
    def encode(
        ...
        lengths: Tensor,
        time_values: Optional[Tensor] = None,
    ) -> Tensor:
        ...
        embeddings = apply_sequence_mask(embeddings, attention_mask)

        transformer_output = self.transformer_encoder(
            embeddings,
            attention_mask=attention_mask,
        )
        _, lstm_last_hidden = self.sequence_encoder(transformer_output, lengths=lengths)
        transformer_pooled = self.masked_mean_pooling(transformer_output, attention_mask)
        return torch.cat([transformer_pooled, lstm_last_hidden], dim=-1)
